    ...
    if st.sidebar.button("导出数据到Neo4j并构建图谱"):
        if neo4j_conn and st.session_state.file_indices:
            try:
                ...
                # 验证导入的数据
                node_count = neo4j_conn.query("MATCH (n:TextChunk) RETURN count(n) as count")[0]['count']
                relationship_count = neo4j_conn.query("MATCH ()-[r:NEXT]->() RETURN count(r) as count")[0]['count']
                
                st.sidebar.success(f"成功导出数据到Neo4j并构建图谱。")
                st.sidebar.info(f"导入的文件数量: {len(st.session_state.file_indices)}")
                st.sidebar.info(f"创建的TextChunk节点数量: {node_count}")
                st.sidebar.info(f"创建的NEXT关系数量: {relationship_count}")
                ...
                if sample_nodes:
                    ...
                    st.sidebar.info("No nodes found.")
